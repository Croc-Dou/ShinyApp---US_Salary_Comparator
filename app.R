...
# Author: Doudou Zhang 
# Data source: https://www.foreignlaborcert.doleta.gov/performancedata.cfm
###############################################################################
...
body <- dashboardBody(
  tabItems(
    ...
    tabItem("myTabForIntroduction",
            ...
            fluidRow(
              box(
                title = "About the Author", solidHeader = TRUE, 
                status = "info", width = 12, collapsible = TRUE,
                column(12, 
                       tags$div(
                         fluidRow(
                           column(4, tags$img(src="doudou.jpg", height=300, width=300)),
                           column(8, tags$div("PROFILE", style = "font-size:16px"),
                                  br(),
                                  tags$li(tags$strong("Doudou Zhang:"), "She is an analytical thinker with business acumen. Her first love was STEM, which she pursued in 
                                          earning a bachelor's degree in Engineering.On that basis, she decided to take a deep dive into data science by enrolling in Analytics Program at Georgetown.
                                          Currently, she works at Freddie Mac modeling group as a Quantitative Analytics Intern. During her seven years in quantitative field, she gained expertise in 
                                          Statistical analysis, Machine learning, Data Visualization and Natural Language Processing (NLP). She is looking forward to utilizing her business domain knowledge in combination with her analytical skills 
                                          to help companies manage, analyze and visualize data and create more value.")
                           )
                         )
                       ),
                       br(),
                       tags$li("If you have any suggestion, question, or review for this app, comments are open! 
                       Please send an email to dz181@georgetown.edu and refer to this Shiny app.")
                )
                ),
                br()
              )
    ...
    tabItem("myTabForScatterPlot",
            h2("Salary Data Box Plot"),
            fluidRow(
              box(
                title = "How to Use", solidHeader = TRUE,
                status = "warning", width = 12, collapsible = TRUE, collapsed = TRUE,
                h5("This 'Salary Box Plot' panel shows the salary distribution by 8 different jobs. It comprises of three sections: a option input section, a box plot section, and an aggreate summary box section.")
              )
            ),
           fluidRow(
              column(4, 
    ...
    tabItem("myTabForGvisMap",
            ...
            fluidRow(
              box(
                title = "How to Use", solidHeader = TRUE,
                status="warning", width=12, collapsible = TRUE, collapsed = TRUE,
                h5("The Salary Comparison Map provides a way to compare salary distribution and the amount of observation of two professions in the United States. You can choose two professions (job titles), then the distribution map and data table will show the updated result.")
              )
            ),
            fluidRow(
              column(3, 
    ...
    tabItem("myTabForDataTable",
            ...
            fluidRow(
              box(
                title = "How to Use", solidHeader = TRUE,
                status="warning", width=12, collapsible = TRUE, collapsed = TRUE,
                h5("This 'Salary Data Explorer' is a data table having features of filtering, paginating, searching,sorting and downloading to explore the data of your interests. 
                   You can interactively choose the options, then the table shows updated result. The data of the table can be filtered by profession (multiple choices), state, salary range, and name (of city and employer), then you can download the selected data table for further analysis.")
              )
            ),
            fluidRow(
              column(4,
    ...
    tabItem("myTabForRecruitRanking",
            ...
            fluidRow(
              box(
                title = "How to Use", solidHeader = TRUE,
                status = "warning", width = 12, collapsible = TRUE, collapsed = TRUE,
                h5("The Top Recruiter Tables panel comprises of two histograms displaying the top 5 companies in salary/headcount, and 5 salary data tables showing who the top recruiters are for each profession. Each table contains employer names, the number of jobs, average salary, the minimum salary, the 25% quantile salary, median salary, the 75% quantile salary, and the maximum salary. ")
              )
            ),
            br(),
            fluidRow(
  ...
  output$myQScatterChart <- renderPlot({
    ...
    if(input$checkboxForShowDataPoint == T){
      ggplot(data = dataForScatterPlot, aes(x = JOB_TITLE_SUBGROUP, y = PAID_WAGE_PER_YEAR/1000, color = JOB_TITLE_SUBGROUP)) + 
        ...
        theme(legend.position="none", legend.title = element_blank(), plot.title = element_text(size=18, face="bold", hjust = 0.5), axis.text = element_text(size = 12), axis.title=element_text(size=15)) +
        xlab("Job Title") +
      ...
      ggplot(data = dataForScatterPlot, aes(x = JOB_TITLE_SUBGROUP, y = PAID_WAGE_PER_YEAR/1000, color = JOB_TITLE_SUBGROUP)) +
        geom_boxplot() +
        ggtitle("Salary VS. Jobs") +
        theme(legend.position="none", legend.title = element_blank(), plot.title = element_text(size=15, face="bold", hjust = 0.5), axis.text = element_text(size = 12), axis.title=element_text(size=15)) +
        xlab("Job Title") +
        ylab("Paid Wage Per Year ($ K)") +
        labs(fill = "") +
        coord_flip()
